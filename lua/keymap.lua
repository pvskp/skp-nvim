...
vim.api.nvim_create_autocmd('User', {
  ...
  callback = function()
    ...
    Map('n', '<leader>f', function() MiniPick.builtin.files() end, { desc = "Pick files" })
    Map('n', '<leader>h', function() MiniPick.builtin.help() end, { desc = "Pick help" })
    Map('n', '<leader>sh', function() MiniExtra.pickers.hl_groups() end, { desc = "Pick help" })
    Map('n', '<leader>g', function() MiniPick.builtin.grep_live() end, { desc = "Live Grep" })
    Map('n', '<leader>b', function() MiniPick.builtin.buffers() end, { desc = "Live Grep" })
    Map({ 'n', 'v' }, '<leader>sw', function()
      MiniPick.builtin.grep({ pattern = vim.fn.expand('<cword>') })
    end, { desc = "Search for word under cursor" })

    Map('n', '<c-e>', function() MiniFiles.open(nil, false) end, { desc = "Pick files" })
  end
})
...
vim.api.nvim_create_autocmd('LspAttach', {
  callback = function(args)
    ...
    if mini_available and extra_available then
      definition_func = function() MiniExtra.pickers.lsp({ scope = 'definition' }) end
      references_func = function() MiniExtra.pickers.lsp({ scope = 'references' }) end
    end
