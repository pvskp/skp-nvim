...
M.execute = function(keymap)
  ...
  ai.select_textobject(inner_or_outer, match_char, { vis_mode = 'v' })
end

M.execute_next = function(keymap)
  M.last_action = keymap
  local inner_or_outer, match_char = M.unpack_pattern(keymap)
  ai.select_textobject(inner_or_outer, match_char, { vis_mode = 'v', search_method = 'next' })
end

M.execute_prev = function(keymap)
  M.last_action = keymap
  local inner_or_outer, match_char = M.unpack_pattern(keymap)
  ai.select_textobject(inner_or_outer, match_char, { vis_mode = 'v', search_method = 'prev' })
end
